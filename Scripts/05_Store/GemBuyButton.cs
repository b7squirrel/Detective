...
/// <summary>
/// 크리스탈 구매 버튼 (현재는 테스트용, 추후 IAP 연동 예정)
/// </summary>
public class GemBuyButton : MonoBehaviour
{
    ...
    void UpdateUI()
    {
        ...
        Logger.Log($"[GemBuyButton] 버튼 업데이트: {productData.ProductName}");
        Logger.Log($"[GemBuyButton] - PurchaseType: {productData.PurchaseType}");
        Logger.Log($"[GemBuyButton] - PurchaseCost: {productData.PurchaseCost}");
        Logger.Log($"[GemBuyButton] - RewardCristal: {productData.RewardCristal}");
    ...
    /// FX 재생 위치 반환
    /// </summary>
    public RectTransform GetGemPoint() => gemPoint;
}