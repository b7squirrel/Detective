...
public class SkillUI : MonoBehaviour
{
    ...
    public void Init(SkillData skillData, int evoStage, float maxSliderValue)
    {
        ...
        UpdateSkillName();
        LocalizationManager.OnLanguageChanged += UpdateSkillName;
    ...
    {
        if (skillName == null || LocalizationManager.Char == null) return;
        int index = (int)skillType / 100 - 1;
        if (index < 0 || index >= LocalizationManager.Char.skillNames.Length) return;
        skillName.text = LocalizationManager.Char.skillNames[index];
    }
