...
/// <summary>
/// 알을 드롭할 수 있는 오브젝트 전용 드롭 시스템
/// 알 중복 생성 방지 로직 포함
/// </summary>
public class EggDrop : DropOnDestroy
{
    ...
    public override void CheckDrop()
    {
        ...
        if (toDrop == null)
        {
            Debug.LogWarning("EggDrop, drop Item Prefab이 null입니다.");
            return;
        }
    ...
    /// <summary>
    /// 알 스폰 가능 여부 체크 (중복 생성 방지)
    /// </summary>
    protected override bool CanSpawnEgg(GameObject item)
    {
        if (item.GetComponent<EggPickUpObject>() != null)
        {
            bool spawnable = GameManager.instance.fieldItemSpawner.isEggSpawnable();
            if (!spawnable)
            {
                Debug.Log("EggDrop: 알 중복 생성 방지 - 스폰 취소");
                return false;
            }
        }
        return true;
